...
export default class TopFoodComponent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      error: null,
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.cityName !== prevProps.cityName) {
      ...
      this.setState({ cityName: this.props.cityName }, function() {
        ...
        if (cityName) {
          fetch(
            `https://www.triposo.com/api/latest/poi.json?tag_labels=foodexperiences|food|cuisine-Street_food|wineries|dinner|breakfast|lunch&location_id=${cityName}&order_by=-food_score&account=Y7QSXUGK&token=kqsq9h5y5bayszmu5cxem9cwg4ly49vu`
          )
            .then(res => res.json())
            .then(
              result => {
                  ...
                  >
                    <div key={item.name} class="personal-info-wrap">
                      <div class="widget-head-info-box">
                        ...
                        {/* <h6>{item.images[0].source_url}</h6> */}
                        {/* <img
