  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.cityName !== prevProps.cityName) {
      ...
      this.setState({ cityName: this.props.cityName }, function() {
        ...
        if (cityName) {
          fetch(
            `https://www.triposo.com/api/latest/poi.json?tag_labels=cinema|culture|history|feature-Live_music|markets|music|literature&location_id=${cityName}&account=Y7QSXUGK&token=kqsq9h5y5bayszmu5cxem9cwg4ly49vu`
          )
            .then(res => res.json())
            .then(
              result => {
