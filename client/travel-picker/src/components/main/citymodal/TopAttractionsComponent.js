...
export default class TopAttractions extends Component {
  constructor(props) {
    super(props);
    this.state = {
      error: null,
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.cityName !== prevProps.cityName) {
      ...
      this.setState({ cityName: this.props.cityName }, function() {
        ...
        if (cityName) {
          fetch(
            `https://www.triposo.com/api/latest/poi.json?tag_labels=air|tours|topattractions&location_id=${cityName}&order_by=topattractions_score&account=Y7QSXUGK&token=kqsq9h5y5bayszmu5cxem9cwg4ly49vu`
          )
            .then(res => res.json())
            .then(
              result => {
        ...
        <div>
          <div class="author-area-pro" style={{ marginTop: "5%" }}>
            ...
            <div class="container-fluid">
              <div class="row">
                <h2 class="breadcomb-ctn" style={{ color: "white" }}>
                  Top Attractions {this.props.countryName}
                </h2>
                {items.map(item => (
                  <div
